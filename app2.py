...
# Google OAuth setup - FIXED VERSION with proper storage
...
def login_google_authorized():
    ...
    if not email:
        flash("No email found in Google response.", "danger")
        return redirect(url_for("login"))
    ...
    if not user:
        ...
        user = User(
            full_name=info.get("name", username_base),
            username=username,
            email=email,
            is_verified=True,
    ...
    # ✅ Save the token AFTER user is known
    google_bp.storage = SQLAlchemyStorage(OAuth, db.session, user=user)
    google_bp.storage.set(google_bp.token)
